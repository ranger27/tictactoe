...
class Board
{
    ...
    char matrix[3][3];
    // // this member function is private as it is sensitive to be allowed outside access.
...
bool Board::fillWith(Player &p, int position)
{
    ...
    {
        cout << "INVALID POSITION. ENTER FROM 1-9." return false;
    }
...
Player& Board::whoWonTheGame(Player &a, Player &b, int row, int column)
{
    ...
    else if (b.sign == matrix[row][column])
    {
        cout << b.name << " is winner..." << endl;
        return b;
    }
